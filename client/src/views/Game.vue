...
export default class Game extends Vue {
      ...
      if (!this.isPaused && !this.gameFinish && this.gameStarted) {
        ...
        this.activeGameShapes.forEach((activeShape: GameShape) => {
          if (!this.moveShape(activeShape, Direction.Down)) {
            ...
            this.activeGameShapes = this.activeGameShapes.filter(
              (gameShape: GameShape) => {
    ...
    window.addEventListener("keydown", (e) => {
      if (this.gameOver || (!this.gameOver && !this.gameStarted)) {
        ...
        if (e.keyCode === 13 && e.ctrlKey) {
          ...
          e.keyCode === 13 &&
          currentTime - gameFinishTime > 1000 &&
    ...
    if (WORD_STORE.includes(testInput)) {
      this.shapesWithKeyword(testInput).forEach((gameShape: GameShape) => {
        if (gameShape.isSuper) {
          this.userScore +=
            50 * this.letterShapeMap[gameShape.shapeName].length;
  ...
  private shapesWithKeyword(keyword: string) {
    const staticKeywordShapes = this.staticGameShapes.filter(
      (gameShape: GameShape) => {
        return gameShape.keyword === keyword;
      }
    );

    const activeKeywordShapes = this.activeGameShapes.filter(
      (gameShape: GameShape) => {
        return gameShape.keyword === keyword;
      }
    );
  ...
  private destroyGameShape(gameShape: GameShape, clearing?: boolean) {
    if (!clearing) {
      const shapeMapIndex: number = this.letterShapeMap[
        gameShape.shapeName
      ].indexOf(gameShape);
      this.letterShapeMap[gameShape.shapeName].splice(shapeMapIndex, 1);
    }

    this.staticGameShapes = this.staticGameShapes.filter(
      (staticShape: GameShape) => {
  ...
  private generateGameShape() {
    ...
    const randIndex: number = ShapeBody.getRandomInt(
      Object.keys(shapes).length
    );
    const shapeName: string = Object.keys(shapes)[randIndex];
    const position: Vector = {
      x: ShapeBody.getRandomInt(this.gameSize.width - 3),
      y: 0,
    };
  ...
  private isColliding(gameShape: GameShape, direction: Direction) {
    for (const currentPosition of gameShape.worldCoords) {
      const deltaPosition: Vector = ShapeBody.appliedVector(
        direction,
        currentPosition
      );

      if (
        ShapeBody.validCoords(
          deltaPosition,
          this.gameSize.width,
        ...
        if (
          !this.coordsAreEmpty(deltaPosition) &&
          !this.coordsAreActiveShape(gameShape, deltaPosition)
        ) {
          return true;
        }
      } else {
  ...
  private coordsAreActiveShape(gameShape: GameShape, vector: Vector) {
    return gameShape.worldCoords.some(
      (sVector: Vector) => sVector.x === vector.x && sVector.y === vector.y
    );
  }
  ...
  get lineTemplate() {
    return {
      ...
      gridTemplateRows: `repeat(${this.numOfLines}, 1fr)`,
    };
  }
